...
class Phonet:
    ...
    def get_phon_wav(self, audio_file, feat_file, file_num, phonclass, plot_flag=True):
        """
        ...
        :param phonclass: phonological class to be evaluated ("consonantal", "back", "anterior", "open", "close", "stop",###"nasal",
        ...
        if phonclass == 'all':
            keys_val=["consonantal", "back", "anterior", "open", "close", "stop", "nasal", "stop", "continuant",
        ...
        lang = audio_file.split('/')[-3]
        utter = audio_file.split('/')[-2]
        ...
        for l in range(len(Models)):
            ...
            if plot_flag:
                ...
                plt.title("["+lang+" "+utter+"] File Number: "+str(file_num))
                plt.legend()
#                 plt.grid()
        df2=pd.DataFrame(df)
